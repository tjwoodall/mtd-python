...
# These are HMRC internal APIs that we don't want to waste time fixing all the errors
HMRC_INTERNAL := \
	mobile-audit \
	mobile-help-to-save \
	mobile-messages \
...
APIS := $(filter-out $(HMRC_INTERNAL), \
		$(sort $(foreach p,$(shell [[ -f $(ALLAPIHTML) ]] && tools/get-api-links.py $(ALLAPIHTML)),$(word 5,$(subst /, ,$(p))))))
...
define KEEP_API_ARTIFACTS

	    artifacts/$(1).*) ;;
	    artifacts/$(1)-main.html) ;;
	    artifacts/$(1)-main.html.change-stamp) ;;
	    artifacts/docs/$(1).*) ;;
	    artifacts/docs/redoc.standalone.js) ;;
	    artifacts/docs/logo-mini.svg) ;;
endef

define KEEP_WEBSCRAPE_VERSIONS

	    resources/public/api/conf/$(1)) ;;
	    redocly.resolved.$(1).yaml) ;;
	    resolved.$(1).yaml) ;;
endef

define KEEP_WEBSCRAPE_ARTIFACTS

	    artifacts/$(1).$(2).webscrape*) ;;
	    artifacts/$(1).$(2).redocly.json) ;;
	    artifacts/docs/$(1).$(2).webscrape.html) ;;
endef

define KEEP_GIT_ARTIFACTS

	    artifacts/$(1).$(2).json*) ;;
	    artifacts/$(1).$(2).tag) ;;
	    artifacts/$(1).$(2).extraconfig.json) ;;
	    artifacts/docs/$(1).$(2).html) ;;
endef
...
$(1):
	...
	[[ $$$${status} -eq 200 ]] || { sleep 2; status=$$$$( curl -s -o /dev/null -u "$(OWNER):$(GITHUB_TOKEN)" -w "%{http_code}" "https://api.github.com/repos/$(OWNER)/$(1)" ); }
	[[ $$$${status} -eq 200 ]] || clone
	# at this point we know that the repo exists
	  ...
	  (
	    ...
	    if ! git rev-parse --verify --quiet main; then
	      if git rev-parse --verify --quiet origin/main; then
	        ...
	        git branch -f main webscrape.clean
	      fi
	    fi
	    git checkout main
	    status=$$$$( curl -s -o /dev/null -u "$(OWNER):$(GITHUB_TOKEN)" -w "%{http_code}" "https://api.github.com/repos/hmrc/$(1)" )
...
$(1).rebase-main: $(if $(filter $(2),$(VERSIONS)),$(1).rebase-webscrape) | $(1)
	...
	git rev-parse fixup-externalValue-tag &>/dev/null || git tag fixup-externalValue-tag hmrc/main
	if ! git merge-base --is-ancestor hmrc/main main; then
...
$(1).cleanup:
	set +x
	for f in artifacts/$(1).[0-9]* artifacts/docs/$(1).[0-9]*; do
	  case "$$$${f}" in
$(foreach v,$(VERSIONS),$(call KEEP_WEBSCRAPE_ARTIFACTS,$(1),$(v)))
$(foreach v,$(GITVERSIONS),$(call KEEP_GIT_ARTIFACTS,$(1),$(v)))
	    *) echo "CLEANUP $$$${f}"
	       rm "$$$${f}"
	    ;;
	  esac
...
$(1).webscrape: $(foreach v,$(VERSIONS),artifacts/$(1).$(v).webscrape.downloaded)
	for d in $$$$( git -C $(1) ls-tree -d --name-only webscrape.clean resources/public/api/conf/ ); do
	  case "$$$${d}" in
$(foreach v,$(VERSIONS),$(call KEEP_WEBSCRAPE_VERSIONS,$(v)))
	    *) echo CLEANUP "$(1)/$$$$d"
	      (
...
$(1).webscrape-resolved: $(foreach v,$(VERSIONS),artifacts/$(1).$(v).webscrape-resolved.downloaded)
	for d in $$$$( git -C $(1) ls-tree --name-only webscrape.clean | grep '^resolved.' ); do
	  case "$$$${d}" in
$(foreach v,$(VERSIONS),$(call KEEP_WEBSCRAPE_VERSIONS,$(v)))
	    *) echo CLEANUP "$(1)/$$$$d"
	      (
...
$(1).webscrape-redocly: $(foreach v,$(VERSIONS),artifacts/$(1).$(v).webscrape-redocly.downloaded)
	for d in $$$$( git -C $(1) ls-tree --name-only webscrape.clean | grep '^redocly.resolved.' ); do
	  case "$$$${d}" in
$(foreach v,$(VERSIONS),$(call KEEP_WEBSCRAPE_VERSIONS,$(v)))
	    *) echo CLEANUP "$(1)/$$$$d"
	      (
...
.PHONY: cleanup
cleanup: $(foreach p,$(APIS),$(p).cleanup)
	set +x
	for f in artifacts/* artifacts/docs/*; do
	  [[ -f $${f} ]] || continue
	  case $${f} in
	    artifacts/api.html) ;;
	    artifacts/application.json) ;;
	    artifacts/redocly.json) ;;
	    artifacts/webscrape.json) ;;
	$(foreach v,$(APIS),$(call KEEP_API_ARTIFACTS,$(v)))
	    *) echo "CLEANUP $${f}"
	       rm "$${f}"
	    ;;
	  esac
...
.PHONY: everything
everything: json validate examples badformats outbound inbound docs .WAIT cleanup
