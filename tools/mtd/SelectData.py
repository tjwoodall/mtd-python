...
# {{{ SelectData
class SelectData(DataProtocol):
  # {{{ __init__
  @typechecked
  def __init__(self, name:str|None, required:bool, schema:JSONList, response:SpecialTypes.ResponseData):
    #print(f"SelectData {name} {required} {json.dumps(schema)} {response}")
    self.schema = schema
    self.name = name
    self.required = required
    self.values:list[ArrayData.ArrayData|ObjectData.ObjectData|SelectData|ValueData.ValueData] = []
    ...
    if response.valtype == "MissingData":
      if self.required:
        raise SpecialTypes.SchemaValidationError(f"Missing SelectData value '{self.name}' - need required value '{self.schema}'")
      self.hidden = True
      response = SpecialTypes.ResponseData(valtype="UseDefault")

    warn_values:list[ArrayData.ArrayData|ObjectData.ObjectData|SelectData|ValueData.ValueData] = []
    for idx,v in enumerate(schema):
      saved_warnings = GlobalVars.GlobalVars.warnings
      GlobalVars.GlobalVars.warnings = []
      if isinstance(v, list):
        value:ArrayData.ArrayData|ObjectData.ObjectData|SelectData|ValueData.ValueData = SelectData(self.name, True, v, response)
      else:
        try:
          if 'properties' in v or ( 'type' in v and v['type'] == "object"):
            value = ObjectData.ObjectData(None, True, v, response)
          elif 'items' in v or ( 'type' in v and v['type'] == "array" ):
            value = ArrayData.ArrayData(None, True, v, response)
          else:
            value = ValueData.ValueData(None, True, v, response)
  ...
  def __repr__(self) -> str:
    r:str = f"SelectData name={self.name} value={self.value}:"
    for k in self.values:
      joinstr = "*" if k.title == self.value else " "
      r += f"\n{joinstr}SelectData {k.title}\n"
...
#    print(f"render SelectData on row {row+1} name '{self.name}' prefix '{prefix}'")
...
#    print(f"recover SelectData name '{self.name}' from row {row+1}")
    self.row = row
...
#    print(f"recover SelectData name '{self.name}' from row {row+1} to {endrow+1}")
    row += 1
...
#    print(f"get_edit_keys SelectData on row {row+1} name '{self.name}' prefix '{prefix}'")
    prefix = self.calcPrefix(prefix, self.name)
    if self.hidden:
...
#    print(f"SelectData::_dispatch_v({method}) {keys} {self.name}")
    if len(keys) < 1:
    ...
    if len(keys) == 1:
      if method == "delete":
        ...
        ErrorHandler.fatal("f{keys} for {method} in SelectData", {})
      return None
    if self.hidden:
...
#    print(f"SelectData::_dispatch_s({method) {keys} {self.name}")
    if len(keys) < 1:
      ErrorHandler.fatal(f"Keys too short {keys}", {})
    if keys[0] != str(self.name if self.name is not None else ""):
      ErrorHandler.fatal(f"{keys[0]} != {self.name}", {})
...
#    print(f"Looking for {self.value} in {[x.title for x in self.values]}")
    ...
    if len(keys) == 2:
      if method == "edit":
        value = f"{castJstr(keys[1])}"
        if value not in [x.title for x in self.values]:
          ErrorHandler.fatal(f"SelectData: setting {value} but need one from {[x.title for x in self.values]}", {})
        LibreOfficeHelper.dump_to_sheet_cell(sheet, 1, self.row+1, LibreOfficeHelper.CellData(value), len(self.values)==1)
        return None
