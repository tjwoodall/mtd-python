...
#    print(f"ValueData::edit {sheet.Name if sheet is not None else 'NONE'}{keys} row={self.row}")
    ...
    if len(keys) == 2:
      if self.type == "string":
        value = LibreOfficeHelper.CellData(castJstr(keys[1]))
      elif self.type == "integer":
        value = LibreOfficeHelper.CellData(castJint(keys[1]))
      elif self.type == "number":
        value = LibreOfficeHelper.CellData(castJfloat(keys[1]))
      elif self.type == "boolean":
        value = LibreOfficeHelper.CellData(castJbool(keys[1]))
      else:
        ErrorHandler.fatal(f"Unhandled type {self.type}", {})
