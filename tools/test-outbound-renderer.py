...
# {{{ process_one_action
@typechecked
def process_one_action(evt:JSONDict) -> None:
  @typechecked
  ...
  evt["_control"].update({ "_server": "Sandbox", })
  jd(evt)
  GlobalVars.warnings = []

  si = SchemaHelper.getOutboundSchemas(evt, response, header)
  ...
  def check_render_and_restore(si:SheetData) -> None:
    ...
    sx = SchemaHelper.getOutboundSchemas(evt, ResponseData(valtype="UseDefault"), ResponseData(valtype="UseDefault"))
    sx.recover(sheet)
    if GlobalVars.warnings:
      jd({"warnings":GlobalVars.warnings})
    GlobalVars.warnings = []
...
def process_action(action:JSONDict, evt:JSONDict) -> None:
  ...
  for action_key in action:
    ...
    if "requestBody" in body:
      ...
      # No body to this request
      evt.update({'_data': None})
      if GlobalVars.in_partition(count):
        process_one_action(dict(evt))
      count += 1
